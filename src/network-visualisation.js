...
async function go() {
    ...
    for (let i = 0; i < n; i++) {
        nodex[i] = Math.floor(Math.random() * canvas.width);
        nodey[i] = Math.floor(Math.random() * canvas.height);
        nodedx[i] = 0;
        nodedy[i] = 0;
    }
    ...
    while (true) {
        // gravity
        for (let i = 0; i < n; i++) {
            nodedx[i] += gk * (canvas.width/2 - nodex[i]);
            nodedy[i] += gk * (canvas.height/2 - nodey[i]);
        }
        ...
        for (let i = 0; i < n; i++) {
            for (let j = 0; j < n; j++) {
                if (i == j) continue;
                let d = Math.sqrt(Math.pow(nodex[i] - nodex[j], 2) + Math.pow(nodey[i] - nodey[j], 2));
                if (d > 0) d = Math.max(d, eps);
                else d = Math.min(d, -eps);

                let dx = (nodex[i] - nodex[j]) / d;
                let dy = (nodey[i] - nodey[j]) / d;
        ...
        for (let i = 0; i < edges.length; i++) {
            let d = Math.sqrt(Math.pow(nodex[edges[i][0]] - nodex[edges[i][1]], 2) + Math.pow(nodey[edges[i][0]] - nodey[edges[i][1]], 2));
        ...
        // act the forces
        for (let i = 0; i < n; i++) {
            nodex[i] += speed * nodedx[i];
            nodey[i] += speed * nodedy[i];
            nodedx[i] = 0;
            nodedy[i] = 0;
        }
...
function paint() {
    ...
    for (let i = 0; i < edges.length; i++) {
        drawLine(nodex[edges[i][0]], nodey[edges[i][0]], nodex[edges[i][1]], nodey[edges[i][1]]);
    }
    for (let i = 0; i < n; i++) {
        drawCircle(nodex[i], nodey[i]);
    }
}
